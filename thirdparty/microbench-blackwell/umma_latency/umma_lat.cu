// Unified tcgen05 MMA latency microbenchmark
// Measures single-MMA end-to-end latency (issue → commit → wait).
...
void umma_lat_kernel() {
    ...
    // Latency measurement: 100 samples, take median
    ...
    for (int iter = 0, phase = 0; iter < NUM_SAMPLES; iter++) {
        ...
        // Warp 0 issues single MMA (CTA 0 only in 2SM mode)
        if (cta_rank == 0 && warp_id == 0 && elect_sync()) {
            mma(1);  // single MMA (pred=true)
    ...
    // Compute and print median (elected thread only)
    if (cta_rank == 0 && warp_id == 0 && elect_sync()) {
        // Bubble sort (100 elements, fine for single-thread)
        ...
        uint64_t median = (samples[NUM_SAMPLES / 2 - 1] + samples[NUM_SAMPLES / 2]) / 2;
        printf("RESULT,%d,%d,%d,%llu\n",
               MMA_M, MMA_N, MMA_K, (unsigned long long)median);
    }
}
...
int main() {
    ...
    umma_lat_kernel<<<CTA_GROUP, 128, SMEM_SIZE>>>();
