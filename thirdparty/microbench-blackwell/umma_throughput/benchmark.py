#!/usr/bin/env python3
"""
Benchmark unified tcgen05 MMA throughput across configurations.
...
MMA_FORMATS = {
    ...
    1: {'name': 'E4M3', 'k': 32, 'depths': [32, 64, 128, 256, 512]},
    2: {'name': 'S8', 'k': 32, 'depths': [32, 64, 128, 256, 512]},
    3: {'name': 'F4', 'k': 64, 'depths': [64, 128, 256, 512, 1024]},
    4: {'name': 'MXF8', 'k': 32, 'depths': [32, 64, 128, 256, 512], 'mx': True},
    5: {'name': 'MXF4', 'k': 64, 'depths': [64, 128, 256, 512, 1024], 'mx': True},
}
...
CSV_FIELDS = [
    'Format', 'ABLayout', 'CTAGroup', 'M', 'N', 'K',
...
def run_benchmark(m, n, mma_format, depth, cta_group, ab_layout, verbose=False):
    ...
    label = f"{fmt_name} {mode} {cta_group}SM M={m}, N={n}, depth={depth}"
    ...
    try:
        ...
        result = subprocess.run(["./umma_tput.out"], capture_output=True, text=True, timeout=180)
        if result.returncode != 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Benchmark unified tcgen05 MMA throughput')
    ...
    for fmt_info in selected_fmts.values():
        for ab_layout, cta_group in sweep:
            ...
            total_runs += len(mn_configs) * len(fmt_info['depths'])
            ...
            for m, n in mn_configs:
                for depth in fmt_info['depths']:
                    ...
                    if result:
                        ...
                        csv_file.flush()
                        result_count += 1
                        print(f"{fmt_name} {mode} {cta_group}SM M={m:3d}, N={n:3d}, depth={depth:4d}: "
